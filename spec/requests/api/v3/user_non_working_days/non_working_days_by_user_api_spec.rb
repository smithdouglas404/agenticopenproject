...
RSpec.describe API::V3::UserNonWorkingDays::NonWorkingDaysByUserAPI do
  ...
  context "with feature disabled", with_flag: { user_working_times: false } do
    ...
    it "returns 404 for GET /api/v3/users/:user_id/non_working_days" do
      get api_v3_paths.user_non_working_days(target_user.id)
      expect(last_response).to have_http_status(404)
    end

    it "returns 404 for POST /api/v3/users/:user_id/non_working_days" do
      post api_v3_paths.user_non_working_days(target_user.id), {}.to_json, headers
      expect(last_response).to have_http_status(404)
    end

    it "returns 404 for DELETE /api/v3/users/:user_id/non_working_days/:date" do
      delete api_v3_paths.user_non_working_day(target_user.id, non_working_day.date)
      expect(last_response).to have_http_status(404)
    end
  end
