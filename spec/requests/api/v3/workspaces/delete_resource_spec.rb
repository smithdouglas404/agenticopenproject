...
RSpec.describe "API v3 Workspace resource delete", content_type: :json do
  ...
  shared_examples_for "deleting a workspace" do
  ...
  describe "DELETE /api/v3/projects/:id" do
    include_examples "deleting a workspace" do
  ...
  describe "DELETE /api/v3/workspaces/:id" do
    include_examples "deleting a workspace" do
      let(:project) { create(:portfolio) }
      let(:path_id) { project.id }
      let(:path) { api_v3_paths.workspace(path_id) }
    end
  end
end
