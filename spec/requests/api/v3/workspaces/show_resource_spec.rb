...
RSpec.describe "API v3 Workspace resource show", content_type: :json do
  ...
  let(:get_path) { "#{api_v3_paths.send(:root)}/workspaces/#{project.id}" }
  ...
  context "for a logged in user" do
    ...
    it "responds with the correct workspace" do
    ...
    it "links to the parent/ancestor workspace" do
      expect(subject.body)
    ...
    describe "permissions" do
      context "when requesting workspace without sufficient permissions" do
        let(:get_path) { "#{api_v3_paths.send(:root)}/workspaces/#{other_project.id}" }
    ...
    context "when requesting nonexistent workspace" do
      let(:get_path) { "#{api_v3_paths.send(:root)}/workspaces/9999" }
    ...
    context "when not being allowed to see the parent workspace" do
      let!(:parent_memberships) do
    ...
    context "with the workspace being archived/inactive" do
