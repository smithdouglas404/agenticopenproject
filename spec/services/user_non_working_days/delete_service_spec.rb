...
RSpec.describe UserNonWorkingDays::DeleteService do
  ...
  let(:non_working_day) { create(:user_non_working_day, user: target_user) }
  ...
  context "when the current user has manage_own_working_times and owns the record" do
    ...
    let(:non_working_day) { create(:user_non_working_day, user: current_user) }
  ...
  context "when the current user has manage_own_working_times but targets another user's record" do
    ...
    it "is unsuccessful" do
      ...
      expect(UserNonWorkingDay.find_by(id: non_working_day.id)).not_to be_nil
    end
  end
