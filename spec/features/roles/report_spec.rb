...
RSpec.describe "Roles report", :js, :selenium do
  ...
  let!(:role) { create(:global_role, permissions:, name: "Global IT MGMT") }
  ...
  it "allows checking and unchecking by column" do
    ...
    within_region "Project" do
      ...
      col_header = find(:columnheader, "Global IT MGMT")
      col_header.click_on accessible_name: "Toggle all Project permissions for Global IT MGMT role"

      expect(page).to have_checked_field "Assign Create projects permission to Global IT MGMT role"

      # colindex = col_header.ancestor(:row).all(:columnheader).index(col_header)
      # colindex += 1 # gridcell filter expression uses XPath position() which is 1-indexed
      # grid = col_header.find(:xpath, XPath.ancestor[XPath.attr(:role) == "grid"])
      # all_checkboxes = grid.all(:gridcell, colindex:).flat_map { it.all(:field, type: :checkbox, minimum: 1) }
      # expect(all_checkboxes).to all(match_selector(:field, checked: true))

      # col_header.click_on accessible_name: "Toggle all Project permissions for Global IT MGMT role"

      # expect(page).to have_unchecked_field "Assign Create projects permission to Global IT MGMT role"
      # all_checkboxes = grid.all(:gridcell, colindex:).flat_map { it.all(:field, type: :checkbox, minimum: 1) }
      # expect(all_checkboxes).to all(match_selector(:field, unchecked: true))
    end
