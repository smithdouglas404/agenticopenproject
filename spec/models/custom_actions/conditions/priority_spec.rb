...
RSpec.describe CustomActions::Conditions::Priority do
  it_behaves_like "associated custom condition" do
    let(:key) { :priority }

    describe "#allowed_values" do
      it "is the list of all priorities" do
        priorities = Array.new(2) { build_stubbed(:priority) }
        allow(IssuePriority)
    ...
    describe "#fulfilled_by?" do
      let(:work_package) { double("work_package", priority_id: 1) }
      ...
      it "is true if values include work package's priority_id" do
        instance.values = [1]
      ...
      it "is false if values do not include work package's priority_id" do
        instance.values = [5]
