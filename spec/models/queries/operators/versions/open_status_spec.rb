...
RSpec.describe Queries::Operators::Versions::OpenStatus do
  describe ".label_key" do
    it "returns the correct label key" do
      expect(described_class.label_key).to eq("label_open")
    end
  end

  describe ".symbol" do
    it "returns the correct symbol" do
      expect(described_class.symbol).to eq("o")
    end
  end
  ...
  describe ".sql_for_field" do
    it "returns the correct SQL condition" do
      expected_sql = "#{Version.table_name}.status = 'open'"
  ...
  describe ".human_name" do
    it "returns the localized name" do
      expect(described_class.human_name).to eq(I18n.t("label_open"))
    end
  end

  describe ".to_sym" do
    it "returns the symbol as a symbol" do
      expect(described_class.to_sym).to eq(:o)
    end
  end

  describe ".to_query" do
    it "returns the URL-escaped symbol" do
      expect(described_class.to_query).to eq("o")
    end
  end
end
