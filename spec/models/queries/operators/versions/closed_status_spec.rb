...
RSpec.describe Queries::Operators::Versions::ClosedStatus do
  describe ".label_key" do
    it "returns the correct label key" do
      expect(described_class.label_key).to eq("label_closed")
    end
  end
  ...
  describe ".sql_for_field" do
    it "returns the correct SQL condition" do
      expected_sql = "#{Version.table_name}.status = 'closed'"
  ...
  describe ".human_name" do
    it "returns the localized name" do
      expect(described_class.human_name).to eq(I18n.t("label_closed"))
    end
  end
