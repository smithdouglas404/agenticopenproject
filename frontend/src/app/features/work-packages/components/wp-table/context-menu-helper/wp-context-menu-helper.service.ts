...
export class WorkPackageContextMenuHelperService {
  ...
  public getIntersectOfPermittedActions(workPackages:any) {
    ...
    const permittedActions = possibleActions.filter((action:any) => workPackages.every((workPackage:WorkPackageResource) => this.isActionAllowed(workPackage, action)));

    permittedActions.forEach((permittedAction:any) => {
