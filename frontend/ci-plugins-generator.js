...
const LINKED_PLUGINS_STYLES_TEMPLATE = (plugins) => `
// This file is generated by the frontend CI plugin generator.
// It mirrors the output of rake openproject:plugins:register_frontend.
...
const pluginsWithGlobalStyles = allFrontendPlugins.filter(([, pluginPath]) => {
  const styleEntry = path.join(pluginPath, 'frontend', 'module', 'global_styles.sass');
  return fs.existsSync(styleEntry);
});
...
function generatePluginStyles(plugins) {
  ...
  console.log(`Regenerating frontend plugin styles ${fileRegister}.`);

  const result = LINKED_PLUGINS_STYLES_TEMPLATE(plugins);
  fs.writeFileSync(fileRegister, result);
};

generatePluginModule(allFrontendPlugins);
generatePluginStyles(pluginsWithGlobalStyles);
