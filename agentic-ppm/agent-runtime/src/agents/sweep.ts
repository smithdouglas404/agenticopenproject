/**
 * Detector sweep — runs the inference detectors + the OpenProject-authored rules
 * evaluator, records findings in the graph store (deduped), and publishes only
 * NEW findings to the OpenProject inbox. Deterministic: detectors + rules, no LLM.
 *
...
import { recordFinding, setFindingStatus, openFindingSeverityCounts } from '../store/findings.js';
...
export async function runSweep(reason: string): Promise<SweepResult> {
  ...
  try {
    ...
    for (const f of findings) {
      // Dedup FIRST: open findings are skipped, so the hourly sweep doesn't
      // re-publish findings that are already standing.
