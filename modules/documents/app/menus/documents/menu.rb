  ...
  class Menu < Submenu
    ...
    def document_status_options
      [
        ...
        menu_item(title: I18n.t("documents.menu.drafts"), query_params: { status: :draft }),
        menu_item(title: I18n.t("documents.menu.archived"), query_params: { status: :archived })
      ]
    end

    def document_type_options
      DocumentType.pluck(:id, :name).map do |type_id, title|
        menu_item(title:, query_params: { type_id: })
      end
    end
    ...
    def active_filter_count
      @active_filter_count ||= (params.keys & %w[type_id :project_id]).count
    end
  end
end
