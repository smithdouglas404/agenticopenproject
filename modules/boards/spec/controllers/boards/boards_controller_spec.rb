...
RSpec.describe Boards::BoardsController do
  ...
  describe "#create" do
    ...
    context "when creating a status board in Community Edition" do
      before do
        allow(Boards::StatusBoardCreateService).to receive(:new).with(user: user).and_return(service_instance)
      end
      ...
      it "allows creating the board" do
        ...
        expect(Boards::StatusBoardCreateService).to have_received(:new).with(user: user)
    ...
    context "when creating an assignee board in Community Edition" do
      ...
      before do
        allow(Boards::AssigneeBoardCreateService).to receive(:new).with(user: user).and_return(assignee_service_instance)
      end
      ...
      it "allows creating the board" do
        ...
        expect(Boards::AssigneeBoardCreateService).to have_received(:new).with(user: user)
        expect(response).to redirect_to(project_work_package_board_path(project, board))
      end
    end
  end
