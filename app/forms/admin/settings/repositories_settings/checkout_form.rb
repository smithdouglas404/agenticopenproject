      ...
      class CheckoutForm < ApplicationForm
        ...
        form do |f|
          f.check_box(
            ...
            checked: Setting.repository_checkout_data[vendor.to_s]["enabled"].to_i > 0,
            ...
            setting_repository_checkout_check_box.nested_form(
              classes: ["mt-2", { "d-none" => Setting.repository_checkout_data[vendor.to_s]["enabled"].to_i.zero? }],
              data: {
                show_when_checked_target: "effect",
                target_name: "#{vendor}_enabled"
              }
            ) do |builder|
              ProfileForm.new(builder, vendor:)
            end
          end
        end
      end
